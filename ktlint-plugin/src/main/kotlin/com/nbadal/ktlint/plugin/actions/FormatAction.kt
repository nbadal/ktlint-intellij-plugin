...
class FormatAction : AnAction() {
    ...
    private fun AnActionEvent.showMenuOptionFormatWithKtlint() =
        getData(CommonDataKeys.PROJECT)
            ?.run { isEnabled(KtlintFeature.SHOW_MENU_OPTION_FORMAT_WITH_KTLINT) }
            ?: false
        ...
        val message =
            listOfNotNull(
                ...
                "Get more value out of ktlint by enabling automatic formatting by using the 'distract free' mode."
                    .takeUnless { project.ktlintMode() == KtlintMode.DISTRACT_FREE },
            ).joinToString(separator = " ")
        when (ktlintFormatContentIterator.status) {
            KtlintFormatContentIterator.BatchStatus.SUCCESS -> {
                KtlintNotifier.notifyInformation(
            ...
            KtlintFormatContentIterator.BatchStatus.FILE_RELATED_ERROR -> {
    ...
    private class KtlintFormatContentIterator(
        ...
        var status = BatchStatus.SUCCESS
        ...
        override fun processFile(fileOrDir: VirtualFile): Boolean {
            ...
            when (ktlintResult?.status) {
                ...
                KtlintRuleEngineWrapper.KtlintResult.Status.FILE_RELATED_ERROR -> {
                    status = BatchStatus.FILE_RELATED_ERROR
