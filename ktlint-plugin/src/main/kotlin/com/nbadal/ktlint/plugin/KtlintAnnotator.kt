...
import com.nbadal.ktlint.plugin.actions.ShowAllKtlintViolationsIntention
...
internal class KtlintAnnotator : ExternalAnnotator<List<LintError>, List<LintError>>() {
    ...
    override fun collectInformation(
        ...
        when {
            hasErrors -> {
                ...
                if (psiFile.project.isEnabled(KtlintFeature.DISPLAY_ALL_VIOLATIONS) ||
            ...
            psiFile.project.isEnabled(KtlintFeature.DISPLAY_VIOLATION_WHICH_CAN_NOT_BE_AUTOCORRECTED_AS_ERROR) ||
                psiFile.project.isEnabled(KtlintFeature.DISPLAY_ALL_VIOLATIONS) ||
                psiFile.project.isEnabled(KtlintFeature.SHOW_PROBLEM_WITH_NUMBER_OF_KTLINT_VIOLATIONS_THAT_CAN_BE_AUTOCORRECTED) ||
                (
                    psiFile.project.isEnabled(KtlintFeature.AUTOMATICALLY_DISPLAY_BANNER_WITH_NUMBER_OF_VIOLATIONS_FOUND) &&
    ...
    override fun apply(
        ...
        val displayAllKtlintViolations =
            psiFile.project.isEnabled(KtlintFeature.DISPLAY_ALL_VIOLATIONS) || psiFile.displayAllKtlintViolations
        val ignoreViolationsPredicate: (LintError) -> Boolean =
            if (psiFile.project.isEnabled(KtlintFeature.SHOW_PROBLEM_WITH_NUMBER_OF_KTLINT_VIOLATIONS_THAT_CAN_BE_AUTOCORRECTED) ||
                (
                    psiFile.project.isEnabled(KtlintFeature.DISPLAY_VIOLATION_WHICH_CAN_NOT_BE_AUTOCORRECTED_AS_ERROR) &&
                        !psiFile.project.isEnabled(KtlintFeature.DISPLAY_ALL_VIOLATIONS)
                )
                ...
                newSilentAnnotation(WARNING)
                    ...
                    .tooltip(
                        "<i>${lintError.errorMessage()}</i><p>" +
                            ...
                            "'${psiFile.project.ktlintVersion().label}' of the ruleset. <strong>Upgrade the ruleset in the " +
    ...
    private fun AnnotationHolder.createSuppressIntention(
        ...
        val severity =
            when {
                ...
                psiFile.project.isEnabled(KtlintFeature.DISPLAY_VIOLATION_WHICH_CAN_NOT_BE_AUTOCORRECTED_AS_ERROR) -> ERROR
                else -> WARNING
            }
