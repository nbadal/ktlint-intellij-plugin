...
// Rewrites bytecode references to relocated packages when loading classes from custom ruleset JARs. Custom rulesets reference non-relocated
// packages (e.g. `org.jetbrains.kotlin`), but ktlint-lib relocates these to `shadow.*` via ShadowJar.
// This class uses (the Intellij IDEA specific fork of) ASM's ClassRemapper to transform references at load time. As of that the class must
// live in the "ktlint-module" namespace.
