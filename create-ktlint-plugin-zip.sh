...
INSTRUMENTED_JAR_FILE_NAME=$(find ktlint-plugin/build/idea-sandbox/plugins -name instrumented*.jar)
if [ $(echo ${INSTRUMENTED_JAR_FILE_NAME} | wc -l | cut -w -f 2) -eq 1 ]; then
  ...
  (cd ktlint-plugin/build/idea-sandbox/plugins && zip  -r ../../${PLUGIN_ZIP_FILE_NAME} ktlint)
